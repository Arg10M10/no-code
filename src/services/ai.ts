...
async function callEdgeFunction(params: CallApiParams): Promise<string> {
  ...
  const response = await fetch(functionUrl, {
      method: 'POST',
      headers,
      body: JSON.stringify({ provider, model, messages, apiKey, system, temperature: temperature ?? 0.7 }),
      signal
  });
...
async function callDirectBrowser(params: CallApiParams): Promise<string> {
  ...
  if (provider === "openai" || provider === "openrouter") {
      ...
      const r = await fetch(url, {
        method: "POST",
        headers,
        body: JSON.stringify({ model, messages, temperature: temperature ?? 0.7, max_tokens: 4096, stream: true }),
        signal,
      });
      if (!r.ok) { const text = await r.text(); throw new Error(`${provider} error: ${text}`); }
      let fullText = "";
      await streamReader(r, (chunk) => { fullText += chunk; onProgress?.(fullText); }, provider);
      return fullText;
  }
  ...
  if (provider === "anthropic") {
      ...
      const chatMessages = messages.filter((m) => m.role !== "system").map((m) => ({ 
        role: m.role, content: typeof m.content === 'string' ? m.content : (m.content as any)
      }));
      const r = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: { "content-type": "application/json", "x-api-key": apiKey, "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true" },
        body: JSON.stringify({ model, max_tokens: 4096, system: systemPrompt, messages: chatMessages, temperature: temperature ?? 0.7, stream: true }),
        signal,
      });
      if (!r.ok) { const text = await r.text(); throw new Error(`Anthropic error: ${text}`); }
      let fullText = "";
      await streamReader(r, (chunk) => { fullText += chunk; onProgress?.(fullText); }, provider);
      return fullText;
  }
  ...
  if (provider === "google") {
      ...
      const r = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ contents, generationConfig: { temperature: temperature ?? 0.7 }, system_instruction: systemPrompt ? { parts: [{ text: systemPrompt }] } : undefined }), signal });
      if (!r.ok) { const text = await r.text(); throw new Error(`Google error: ${text}`); }
...
async function callApi(params: CallApiParams): Promise<string> {
  try { return await callEdgeFunction(params); } catch (e) { return await callDirectBrowser(params); }
}
...
IMPORTANTE:
1. First, you MUST explain your plan and reasoning in plain text. **You MUST use the same language as the user's request**.
...
`;
  ...
  if (images && images.length > 0) {
    const userContent: ChatContent = [{ type: 'text', text: `Recreate this. ${prompt}` }];
  ...
  if (codeContext) {
    const userPrompt = `Based on current files, apply: "${prompt}"\n\nFiles:\n${codeContext}`;
...
export async function generateAnswer(req: { 
  ...
  onCodeStreamUpdate?: (codeStream: string) => void;
  ...
  try {
    ...
    throw new Error(`AI parse error. Please retry.`);
  }
}
